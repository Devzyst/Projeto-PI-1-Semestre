...
def coluna_existe(cursor, tabela, coluna):
    """
    Verifica se uma coluna existe em uma tabela do banco atual.

    Args:
        cursor (mysql.connector.cursor.MySQLCursor): Cursor ativo do banco de dados.
        tabela (str): Nome da tabela que sera consultada.
        coluna (str): Nome da coluna que sera verificada.

    Returns:
        str: Retorna "SIM" se a coluna existir ou "NAO" caso contrario.
    """
    cursor.execute("""
...
def realizar_zerezima(cursor):
    ...
    if coluna_existe(cursor, "candidatos", "votos") == "SIM":
...
def registrar_voto():
    ...
    try:
        ...
        if candidato and coluna_existe(cursor, "candidatos", "votos") == "SIM":
            cursor.execute("""
                UPDATE candidatos
                SET votos = votos + 1
                WHERE id = %s
